...
// If dynamic loading is disabled we can just include CUDA runtime and
...
static inline const char *cuda_GetErrorString(CUresult result) {
    ...
    // Covers all possible errors with the nice explanations.
    return cuewErrorString(result);
#else
    // Fallback to less user friendly messages which only shows
    // the error code, not the verbal explanation.
    switch (result) {
